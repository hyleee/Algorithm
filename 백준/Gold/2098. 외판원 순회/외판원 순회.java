...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        map = new int[N][N];
        dp = new int[N][(1 << N)]; // 가능한 모든 방문상태는 0부터 (1 << N) - 1까지.
        ...
        for (int i = 0; i < N; i++) {
            Arrays.fill(dp[i], -1); // 아직 계산해보지 않은 경로는 최대 비용으로 표시해두기
        }
    ...
    static int travel(int cityIdx, int visited) {
        // 모든 도시를 방문했다면
        if (visited == (1 << N) - 1) {
        ...
        // 이미 한 번 비용이 계산된 루트라면 (dp 값이 존재하는 경우)
        if (dp[cityIdx][visited] != -1) {
            return dp[cityIdx][visited]; // 그대로 반환
        }
        dp[cityIdx][visited] = INF;

        // 현재 도시에서 각 n의 도시로 이동한 경우에 대해 DFS
        for (int n = 0; n < N; n++) {
            // visited & (1 << n) == 0 : 도시 n이 아직 방문되지 않았다면
            // map[cityIdx][n] != 0 : 현재 도시에서 도시 n으로의 경로가 존재한다면
            if ((visited & (1 << n)) == 0 && map[cityIdx][n] != 0) {
                ...
                dp[cityIdx][visited] = Math.min(
                        ...
                        travel(n, visited | (1 << n)) + map[cityIdx][n]); // 다음 도시로 이동한 후 비용
